...
setup_notifications() {
    ...
    echo ""
    echo "  ${CYAN}1${NC}) Slack"
    echo "  ${CYAN}2${NC}) Discord"
    echo "  ${CYAN}3${NC}) Telegram"
    echo "  ${CYAN}4${NC}) Custom script"
    echo "  ${CYAN}5${NC}) Skip for now"
    echo ""
...
setup_telegram_interactive() {
    ...
    echo "  1. Message ${CYAN}@BotFather${NC} on Telegram"
    ...
    if [[ -n "$TELEGRAM_TOKEN" ]]; then
        ...
        echo "  3. Visit: ${CYAN}https://api.telegram.org/bot$TELEGRAM_TOKEN/getUpdates${NC}"
